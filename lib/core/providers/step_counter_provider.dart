...
class StepCounterProvider with ChangeNotifier {
  ...
  // Track last update time for lifecycle management
  ...
  Future<void> _performAsyncInitialization() async {
    ...
    try {
      ...
      if (_isStepDetectionAvailable) {
        // Set up callback for receiving step updates from Kotlin service
        StepTrackingService().addStepCountCallback(_onKotlinStepUpdate);
        StepTrackingService().start();
        
        // Start the Android foreground service for persistent notification
        await _startPersistentStepNotification();
  ...
  /// Save step data to local storage with throttling to improve performance
  Future<void> _saveStepDataToLocal() async {
    try {
      // PERFORMANCE FIX: Throttle saves to avoid excessive disk I/O
      final now = DateTime.now();
      if (_lastSaveTime != null && now.difference(_lastSaveTime!).inSeconds < 10) {
        _debugLog('Throttling save - too frequent');
        return; // Don't save more than once every 10 seconds
      }
  ...
  // Handle step count updates from Kotlin service
  void _onKotlinStepUpdate(int stepCount) {
    _debugLog('Received step count from Kotlin service: $stepCount');
    
    // CRITICAL FIX: Only update if count is valid and reasonable
    // Prevent unnecessary resets - preserve existing data if new count is suspicious
    if (stepCount >= 0) {
      // Check if this is a reasonable update
      if (_todaySteps > 0 && stepCount < _todaySteps && (_todaySteps - stepCount) > 1000) {
        _debugLog('Ignoring suspicious step count decrease from Kotlin: $stepCount < $_todaySteps');
        return; // Don't overwrite good data with suspicious data
      }
      
      // Only update if the new count is greater than current or if current is 0
      if (stepCount >= _todaySteps || _todaySteps == 0) {
        _todaySteps = stepCount;
        
        // Update today's data in weekly array
        _updateTodayStepDataFromKotlin(stepCount);
        ...
        _debugLog('Step count updated from Kotlin: $_todaySteps');
      } else {
        _debugLog('Ignoring step count that would decrease progress: $stepCount < $_todaySteps');
      }
    } else {
      _debugLog('Ignoring invalid step count from Kotlin: $stepCount');
    }
  }

  // Update today's step data from Kotlin service
  void _updateTodayStepDataFromKotlin(int stepCount) async {
    try {
      final today = DateTime.now();
      ...
      _errorLog('Error updating step data from Kotlin: $e');
    }
  }

  // Initialize pedometer streams (fallback for devices without Kotlin service)
  void _initializePedometer() {
    // Primary step counting is handled by Kotlin service
    // This is kept as fallback for iOS or devices where Kotlin service fails
    if (!_isStepDetectionAvailable) {
      _debugLog('Step detection not available, skipping pedometer initialization');
      return;
    }
    
    _debugLog('Initializing pedometer as fallback for Kotlin service');
    ...
    try {
      // Only listen to pedestrian status for walking detection
      ...
      _debugLog('Pedestrian status stream initialized successfully');
    } catch (e) {
  ...
  int _calculateDailySteps(int currentDeviceSteps, int previousDeviceSteps) {
    _debugLog('Calculating daily steps - current: $currentDeviceSteps, previous: $previousDeviceSteps');
    ...
    if (previousDeviceSteps == 0) {
      ...
      if (yesterdayData.deviceStepsAtSave > 0) {
        ...
        _debugLog('Using yesterday baseline - daily steps: $dailySteps');
        return dailySteps > 0 ? dailySteps : 0; // Start at 0 for new day, not device count
      } else {
        // No yesterday data - this is a new user or fresh install
        // Start daily count at 0 and save current device count as baseline
        _debugLog('No previous data - starting at 0 steps for new user/day');
        return 0; // FIXED: Always start new users at 0, never use device cumulative count
      }
    } else {
      ...
      _debugLog('Normal daily calculation - daily steps: $dailySteps');
      return dailySteps > 0 ? dailySteps : 0; // Ensure non-negative
    }
  }
  ...
  Future<void> _checkForNewDayAsync() async {
    try {
      ...
      if (existingTodayIndex == -1) {
        ...
        _debugLog('New day detected, creating fresh entry for today');
        
        // FIXED: Reset steps to 0 for new day (correct behavior)
        _todaySteps = 0;
        
        // Get the current device step count to use as baseline for the new day
        int currentDeviceSteps = 0;
        
        // If we have yesterday's data, use its final device count as today's baseline
        ...
        // Create a new entry for today starting at 0 steps
        _weeklyStepData.insert(0, DailyStepData(
          date: today,
          steps: 0, // FIXED: Start new day at 0 steps
          goal: 10000,
          deviceStepsAtSave: currentDeviceSteps, // Baseline for calculating daily steps
        ));
        ...
        // Update derived values for the preserved step count
        _updateDerivedValues();
        
        _debugLog('Daily reset complete - today steps preserved: $_todaySteps');
      ...
      _errorLog('Error checking for new day: $e');
    }
  }
  ...
  void forceNewDayReset() {
    _debugLog('FORCING new day reset for testing...');
    _todaySteps = 0;
    ...
    _debugLog('Forced new day reset complete - today\'s steps reset to 0');
  }
  
  // Handle step goal achievement
  void _onStepGoalReached(int steps, int goal) {
    _debugLog('Step goal reached! $steps steps (goal: $goal)');
    ...
    if (_experienceProvider != null) {
      ...
      _debugLog('Step goal reached - XP should be awarded by UI layer');
    }
  }
  ...
  void dispose() {
    ...
    _stopPersistentStepNotification();
    
    // Remove callback from step tracking service to prevent memory leak
    StepTrackingService().removeStepCountCallback(_onKotlinStepUpdate);
  ...
  void handleAppLifecycleChange(AppLifecycleState state) {
    _debugLog('App lifecycle changed to $state');
    
    if (state == AppLifecycleState.resumed) {
      _debugLog('App resumed, checking for new day and reinitializing');
      // CRITICAL FIX: Don't automatically check for new day on resume - preserve current state
      // Only check if we've been away for more than 1 hour
      final now = DateTime.now();
      if (_lastUpdateTime == null || now.difference(_lastUpdateTime!).inHours >= 1) {
        checkForNewDay();
      }
      ...
      _debugLog('App paused/inactive/detached, ensuring steps are saved');
      // Stop the periodic timer to save battery when app is in background
      _stopNewDayCheckTimer();
      
      // CRITICAL FIX: Always save current data before going to background
      _saveStepDataToLocal();
      
      // Make sure our step tracking service is running in background
      StepTrackingService().start();
      
      // Update timestamp for lifecycle tracking
  ...
  void _startNewDayCheckTimer() {
    ...
    _newDayCheckTimer = Timer.periodic(const Duration(hours: 1), (timer) {
      _debugLog('Periodic new day check triggered');
      checkForNewDay();
    });
    
    _debugLog('Started periodic new day check timer (every hour)');
  }
  ...
  void _stopNewDayCheckTimer() {
    ...
    _debugLog('Stopped periodic new day check timer');
  }
  ...
  /// Start persistent step notification using Android foreground service
  Future<void> _startPersistentStepNotification() async {
    try {
      final platform = MethodChannel('step_counter_service');
      await platform.invokeMethod('startStepService');
      _debugLog('Started persistent step notification service');
    } catch (e) {
      _debugLog('Failed to start persistent step notification: $e');
    }
  }

  /// Stop persistent step notification
  Future<void> _stopPersistentStepNotification() async {
    try {
      final platform = MethodChannel('step_counter_service');
      await platform.invokeMethod('stopStepService');
      _debugLog('Stopped persistent step notification service');
    } catch (e) {
      _debugLog('Failed to stop persistent step notification: $e');
    }
  }
}
